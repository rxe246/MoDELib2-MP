...
namespace model
{
    ...
    /* init */,uniformTractionController(uniformLoadController? std::unique_ptr<UniformControllerType>(nullptr) : getUniformEDcontroller(this->microstructures.ddBase))
    ...
    /* init */,solverInitialized(false)
{
    ...
    #pragma omp parallel for
            ...
            for(size_t k=0;k<tractionList.size();++k)
            {
                ...
                pt = this->microstructures.ddBase.voigtTraits.v2m(uniformTractionController->flux(this->microstructures.ddBase.simulationParameters.totalTime),false); // setZero();
                for(const auto& microstructure : this->microstructures)
                {
                    if(microstructure.get()!=static_cast<const MicrostructureBase<dim>* const>(this))
                    {// not the ElasticDeformation physics
