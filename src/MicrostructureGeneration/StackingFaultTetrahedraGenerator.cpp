...
bool StackingFaultTetrahedraGenerator::generateSingleSFT(MicrostructureGenerator& mg,const int& planeID,const VectorDimD& basePoint,const bool& inverted,const int& sftSize)
{
    if(sftSize>1.0)
    {
        ...
        if(found.first)
        {
            ...
            {
                const auto basePlane(inverted? std::shared_ptr<GlidePlaneBase>(new GlidePlaneBase(grain->planeNormals().at(planeID)->primitiveVectors.second,grain->planeNormals().at(planeID)->primitiveVectors.first,nullptr)) : grain->planeNormals().at(planeID));
                ...
                const VectorDimD corner(grain->snapToLattice(basePoint-faceBaryShift).cartesian());
...
//                nodePos.push_back((nodePos[0]+nodePos[1]+nodePos[2])/3.0+unitNormal*sftSize*sqrt(2.0/3.0));
                ...
                if(mg.allPointsInGrain(nodePos,grainID))
                {
                    ...
                    {// SFT loop 0->1->2 (base Frank loop)
                        ...
                        const auto n012(grain->reciprocalLatticeDirection((nodePos[1]-nodePos[0]).cross(nodePos[2]-nodePos[1])));
                    ...
                    {// SFT loop 2->3->0 (side Shockley loop)
                        ...
                        const auto n230(grain->reciprocalLatticeDirection((nodePos[3]-nodePos[2]).cross(nodePos[0]-nodePos[3])));
                    ...
                    {// SFT loop 0->3->1 (side Shockley loop)
                        ...
                        const auto n031(grain->reciprocalLatticeDirection((nodePos[3]-nodePos[0]).cross(nodePos[1]-nodePos[3])));
                    ...
                    {// SFT loop 1->3->2 (side Shockley loop)
                        ...
                        const auto n132(grain->reciprocalLatticeDirection((nodePos[3]-nodePos[1]).cross(nodePos[2]-nodePos[3])));
