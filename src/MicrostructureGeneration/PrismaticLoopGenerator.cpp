...
//            const LatticePlaneKey sessilePlaneKey(axis,sessilePlaneHeight,grain->grainID);
            ...
            else if(mg.ddBase.isPeriodicDomain)
            {
...
//                    const LatticePlaneKey planeKey(planes[k2].n,planeHeight,grain->grainID);
                ...
                {
                    VectorDimD A = sessileLoopNodePos[k1];
                    VectorDimD B = sessileLoopNodePos[k2];
                    VectorDimD minBox = mg.ddBase.mesh.xMin();
                    VectorDimD maxBox = mg.ddBase.mesh.xMax();
                    ...
                    for (int i = 0; i < 3; ++i) {
                        if (std::abs(dir[i]) < 1e-12) {
                            if (A[i] < minBox[i] || A[i] > maxBox[i]) intersectsBox = false;
                        } else {
                            double t_min = (minBox[i] - A[i]) / dir[i];
                            double t_max = (maxBox[i] - A[i]) / dir[i];
                            if (t_min > t_max) std::swap(t_min, t_max);
                            t0 = std::max(t0, t_min);
                            t1 = std::min(t1, t_max);
                            if (t0 > t1) intersectsBox = false;
                        }
                    }
                    ...
                    if (intersectsBox) {
                        ...
                        size_t numLoopsBefore = mg.config().loops().size();
                        size_t numNodesBefore = mg.config().loopNodes().size();

                        mg.insertJunctionLoop(loopNodePos,periodicGlidePlane,
                                            b.cartesian(),periodicGlidePlane->referencePlane->unitNormal,
                                            periodicGlidePlane->referencePlane->P,grain->grainID,DislocationLoopIO<3>::GLISSILELOOP);
                        
                        if (mg.config().loops().size() > numLoopsBefore && mg.config().loopNodes().size() == numNodesBefore) {
                            mg.config().loops().pop_back(); 
                        }
                    }
                }
                else if(mg.ddBase.isPeriodicDomain)
