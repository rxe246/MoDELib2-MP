            ...
            { // Check force condition for internal segments
                ...
                if (LA > FLT_EPSILON && LB > FLT_EPSILON)
                {
                    if (ssd.dMin > FLT_EPSILON)
                    {
                        ...
                        else if (linkA->isSessile() && linkA->loopLinks().size() == 1 
                             && linkB->isSessile() && linkB->loopLinks().size() == 1)
                        {
                            // In pure climb, osmotic forces overwhelm elastic repulsion.
                            // If two expanding Frank loops physically touch, they MUST zip to prevent ghost-crossing!
                            isValidJunction = true; 
                        }
                        else 
                        {
                            // Fetch the active ClusterDynamics microstructure
                            const auto CD = DN.microstructures.template getUniqueTypedMicrostructure<ClusterDynamics<dim>>();
                            
                            if (CD && linkA->quadraturePoints().size() > 0 && linkB->quadraturePoints().size() > 0) 
                            {
                                // Extract the thermodynamic parameters dynamically from the ClusterDynamicsParameters struct
                                // Note: Verify the exact variable names in your ClusterDynamicsParameters.h 
                                // (Typically stored as kB, T, and Omega)
                                const double kT_Omega = (CD->cdp.kB * CD->cdp.T) / CD->cdp.omega; 

                                // Extract deltaC from the first quadrature point of each link
                                const double deltaC_A = linkA->quadraturePoint(0).cDD(0) - linkA->quadraturePoint(0).cCD(0);
                                const double deltaC_B = linkB->quadraturePoint(0).cDD(0) - linkB->quadraturePoint(0).cCD(0);

                                // Compute Osmotic Forces (Bardeen-Herring chemical force)
                                const VectorDim osmoticOnA = -(kT_Omega * deltaC_A) * linkA->burgers().cross(chordA);
                                const VectorDim osmoticOnB = -(kT_Omega * deltaC_B) * linkB->burgers().cross(chordB);

                                // Evaluate if the TOTAL thermodynamic driving force pushes them together
                                if ((forceOnA + osmoticOnA).dot(dxShift) > FLT_EPSILON && (forceOnB + osmoticOnB).dot(dxShift) < -FLT_EPSILON)
                                {
                                    VerboseJunctions(3, "attractive pair (Total Force)" << std::endl;);
                                    isValidJunction = true; 
                                }
                                else
                                {
    ...
    {/*! @param[in]  avoidNodeIntersection
      ...
      */
        ...
        for(size_t k=0;k<reducedIntersectionPairs.size();++k)
        {
            ...
            if (   linkA->isBoundarySegment() || (linkA->source->isBoundaryNode() && linkA->sink->isBoundaryNode())
                || linkB->isBoundarySegment() || (linkB->source->isBoundaryNode() && linkB->sink->isBoundaryNode()) )
            {
                continue; 
            }
            ...
            for(const auto& pair : pcPlanes)
            {
                ...
                {// a common coincident plane found
                    ...
                    const bool isFrankA = linkA->isSessile() && linkA->loopLinks().size() == 1;
                    const bool isFrankB = linkB->isSessile() && linkB->loopLinks().size() == 1;
                    if(cosTheta<0.7 && !(isFrankA && isFrankB))
            ...
            if (isLink)
            {
                if (!isLink->hasZeroBurgers())
                {
                    ...
                    if (!source->isBoundaryNode() && !sink->isBoundaryNode())
                    {
                        ...
                        networkNodes.push_back(sink);
                        networkNodes.push_back(source);
