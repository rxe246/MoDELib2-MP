...
namespace model
{
    ...
    /*init*/,inverseLatticeBasis(latticeBasis.inverse())
    /*init*/,NX(gridSize(0))
    /*init*/,NY(gridSize(1))
    /*init*/,NZ(gridSize(2))
...
//template <int N>
//void GlidePlaneNoiseBase<N>::averageRealNoise(const int& reps) const
//{
//
//#ifdef _MODEL_GLIDE_PLANE_NOISE_GENERATOR_
//
...
//    std::normal_distribution<REAL_SCALAR> distribution(0.0,1.0);
//
//    std::vector<REAL_SCALAR*> okCorr(originalKCorrelations()); // store original correlations once
//    for(int k=0;k<reps;++k)
//    {
//        std::vector<COMPLEX*> nkCorr(noisyKCorrelations(okCorr,generator,distribution)); // apply noise to correlations
//        std::vector<REAL_SCALAR*> rNoise(realNoise()); // get real
//        // do something (average?) with nkCorr and rNoise
//    }
//
//#endif
//}
