...
typename GlidePlaneBase::VectorLowerDimD GlidePlaneBase::localSlipVector(const VectorDimD& b) const
{
...
double GlidePlaneBase::misfitEnergy(const VectorDimD& b) const
{
    if(gammaSurface)
    {
        ...
        //            throw std::runtime_error("SLIP VECTOR NOT ON GlidePlaneBase");
        //        }
        return gammaSurface->operator()(localSlipVector(b));
    }
    else
    {
        return 0.0;
...
typename GlidePlaneBase::MatrixLowerDim GlidePlaneBase::localBasis() const
{
    MatrixLowerDim temp;
    temp.col(0)=localSlipVector(primitiveVectors.first.cartesian());
    temp.col(1)=localSlipVector(primitiveVectors.second.cartesian());
    return temp;
}
