...
namespace model
{
    ...
    /* init */,isPeriodicEdge(deltaShift.squaredNorm()>FLT_EPSILON && periodicFacesInSameGrain(meshIntersection_in))
    /* init */,outNormal(patch->glidePlane->localDirection(meshIntersection->regionBoundaryNormal(patch->glidePlane->grain.grainID)).normalized())
    /* init */,next(nullptr)
    /* init */,prev(nullptr)
    /* init */,twin(nullptr)
    {
    ...
    bool PeriodicPlaneEdge<dim>::periodicFacesInSameGrain(const std::shared_ptr<const MeshBoundarySegment<dim>> meshIntersection_in)
    {
        bool temp(true);
        for(const auto& face : meshIntersection_in->faces)
        {
            if(face->periodicFacePair.second)
            {// face has periodic face
                temp = (temp && face->regionIDs==face->periodicFacePair.second->regionIDs);
            }
        }
        return temp;
    }
    ...
    template<int dim>
    PeriodicPlaneEdge<dim>* PeriodicPlaneEdge<dim>::nextUntwinned() const
    {/*!\returns the string "i->j" where i is source()->sID and j=sink()->sID
      */
        
        if(next->twin)
        {
            return next->twin->nextUntwinned();
        }
        else
        {
            return next;
    ...
    template<int dim>
    PeriodicPlaneEdge<dim>* PeriodicPlaneEdge<dim>::prevUntwinned() const
    {/*!\returns the string "i->j" where i is source()->sID and j=sink()->sID
      */
        
        if(prev->twin)
        {
            return prev->twin->prevUntwinned();
        }
        else
        {
            return prev;
    ...
    std::pair<typename PeriodicGlidePlane<dim>::VectorDim,bool> PeriodicGlidePlane<dim>::findPatch(const typename PeriodicGlidePlane<dim>::VectorLowerDim& P0,const VectorDim& guessShift)
    {
        ...
        {
            insideReferencePoint+=referencePlane->localPosition(seg->P0-lastPatch->shift)/centralPlane->meshIntersections.size();
        }
        ...
        while(!lastPatch->contains(P0))
        {
            ...
            switch (crossdEdges.size())
            {
                ...
                default:
                {
                    ...
                    throw std::runtime_error("1 or 2 edges must be crossed");
        ...
        if(!lastPatch->contains(P0))
        {
            throw std::runtime_error("lastPatch does not cointain searched point.");
        }
