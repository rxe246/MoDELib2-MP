        ...
        for(const auto& pair : noiseFiles)
        {
            ...
            if(noiseFileString!="none" && noiseFileString!="None")
            {
                ...
                if(type=="MDSolidSolutionNoise")
                {
                    ...
                    const auto success(solidSolutionNoise().emplace(tag,new MDSolidSolutionNoise(mat,tag,correlationFile_L,correlationFile_T,seed,gridSize,gridSpacing,Eigen::Matrix<double,2,2>::Identity(),a_Cai)));
